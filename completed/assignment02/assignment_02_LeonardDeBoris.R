...
voter_turnout_df1 <-  read_excel('data/G04ResultsDetail2004-11-02.xls', sheet = 
...
voter_turnout_df2 <- read_excel('data/G04ResultsDetail2004-11-02.xls', sheet = 
                                    'Voter Turnout', skip= 1, nrow(2))
...
tables <- dbReadTable('data/tidynomicon/person.csv')
            lapply('table_names', 'dbReadTable', 'conn = db')
tables

## Use the `dbDisconnect` function to disconnect from the database
dbDisconnect(RSQLite::SQLite(),'data/tidynomicon/example.db')
